"""Repository paths for the notebook v2 pipeline (isolated from notebooks/)."""
...
# Legacy v1 sessions (optional import for review queue)
V1_SESSIONS_DIR = REPO_ROOT / "notebooks" / "outputs" / "har_sessions"
...
def ensure_notebook_paths() -> None:
    ...
    for d in (OUTPUTS_DIR, CHECKPOINTS_DIR, MEMORY_DIR, SESSIONS_DIR, HUMAN_LABELS_DIR):
...
def yolo_weights_path() -> Path:
    for p in (
        ...
        REPO_ROOT / "notebooks_v2" / "yolov8n.pt",
    ):
        if p.is_file():
            return p
