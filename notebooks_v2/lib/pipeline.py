"""Orchestrate notebooks v2 pipeline (01→07) with crop-aligned embeddings + HITL merge."""
...
class PipelineConfig:
    ...
    checkpoint_name: str = "har_vjepa_v2_12c_100each.pt"
    eval_video_name: str = "perperson_eval_v2.mp4"
...
def step_data_check(cfg: PipelineConfig) -> dict:
    ...
    if report.error:
        ...
        _log("01", f"Sample: {len(report.clips)} clips, {report.n_classes} classes (v2)")
...
def step_embeddings(cfg: PipelineConfig) -> dict:
    ...
    for rec in tqdm(clips, desc="V-JEPA2 v2"):
...
def run_pipeline(cfg: PipelineConfig) -> dict:
    ...
    results: dict = {"config": asdict(cfg), "cache": cache_info, "steps": {}, "status": "ok", "pipeline_version": "v2"}
