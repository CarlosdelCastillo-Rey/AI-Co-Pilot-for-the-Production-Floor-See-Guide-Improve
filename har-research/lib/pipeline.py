...
from lib.crop_extract import crops_for_embedding, crops_for_embedding_with_meta, save_crop_debug_samples
...
class PipelineConfig:
    ...
    # ── Debug ─────────────────────────────────────────────────────────────────
    viz_crop_samples: int = 0   # >0 → save crop-pipeline PNGs (one per class, up to N)
...
def step_embeddings(cfg: PipelineConfig) -> dict:
    ...
    if cfg.viz_crop_samples > 0:
        viz_dir = OUTPUTS_DIR / "debug_crops"
        paths = save_crop_debug_samples(
            clips, viz_dir,
            n_samples=cfg.viz_crop_samples,
            view=cfg.inhard_view,
            buffer_frames=cfg.buffer_frames,
            seed=cfg.sample_seed,
        )
        _log("02", f"Saved {len(paths)} crop debug PNGs → {viz_dir}/")
