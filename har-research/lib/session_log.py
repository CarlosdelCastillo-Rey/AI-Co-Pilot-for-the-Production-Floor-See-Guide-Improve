...
class HarSessionLogger:
    ...
    def log_inference(
        ...
        *,
        ...
        persist_artifacts = label_changed or save_for_review or self.save_crop_every_event
        save_full_frame = label_changed or save_for_review
        if label_changed:
            self._n_confirmed += 1
        if persist_artifacts:
            label_slug = (prediction.get("label") or prediction.get("raw_label") or "unknown")
            label_slug = str(label_slug).replace(" ", "_").lower()[:32]
            emb_name = f"{event_id}_tid{track_id}.npy"
            crop_name = f"{event_id}_tid{track_id}_crop.jpg"

            if save_full_frame and frame_bgr is not None:
                frame_name = f"f{frame_idx:05d}_tid{track_id}_{label_slug}.jpg"
                annotated = self._annotate_frame(
                    frame_bgr,
                    track_id=track_id,
                    bbox=bbox,
                    label=str(prediction.get("label") or prediction.get("raw_label") or ""),
                    confidence=float(prediction.get("confidence") or prediction.get("raw_confidence") or 0.0),
            ...
            if label_changed or save_for_review:
        ...
        emb_arr = prediction.get("embedding")
        if self._registry is not None and isinstance(emb_arr, np.ndarray):
            gid, score = self._registry.resolve(
                emb_arr,
                session_id=self.session_id,
                track_id=track_id,
                event_id=event_id,
                ...
                confidence=float(prediction.get("confidence") or prediction.get("raw_confidence") or 0),
            )
            self._track_global[track_id] = gid
            row["global_person_id"] = gid
            row["reid_match_score"] = round(score, 4)
            row["person_id"] = gid
        elif track_id in self._track_global:
            row["global_person_id"] = self._track_global[track_id]
            row["person_id"] = self._track_global[track_id]
