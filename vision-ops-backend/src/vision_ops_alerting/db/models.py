...
# ── har-research parity: full audit sessions, Re-ID, HITL ──────────────────


class HarAuditSession(Base):
    """Rich session log matching har-research HarSessionLogger manifest + summary."""

    __tablename__ = "har_audit_sessions"

    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    source: Mapped[str] = mapped_column(String(32), default="eval")  # eval | live | bench
    camera_id: Mapped[str | None] = mapped_column(String(64), index=True)
    video_name: Mapped[str | None] = mapped_column(String(512), index=True)
    model_id: Mapped[str | None] = mapped_column(String(64))
    model_tag: Mapped[str | None] = mapped_column(String(128))
    checkpoint_name: Mapped[str | None] = mapped_column(String(512))
    classifier_version: Mapped[str | None] = mapped_column(String(128))
    embedding_model: Mapped[str | None] = mapped_column(String(256))
    embedding_version: Mapped[str | None] = mapped_column(String(128))
    class_names_json: Mapped[str | None] = mapped_column(Text)
    hyperparams_json: Mapped[str | None] = mapped_column(Text)
    started_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, index=True)
    ended_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    n_events: Mapped[int] = mapped_column(Integer, default=0)
    n_confirmed: Mapped[int] = mapped_column(Integer, default=0)
    n_tracks: Mapped[int] = mapped_column(Integer, default=0)
    summary_json: Mapped[str | None] = mapped_column(Text)

    events: Mapped[list["HarSessionEvent"]] = relationship(
        "HarSessionEvent",
        back_populates="session",
        cascade="all, delete-orphan",
    )
...
class HarSessionEvent(Base):
    """One inference row — mirrors har-research events.jsonl."""

    __tablename__ = "har_session_events"

    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    session_id: Mapped[str] = mapped_column(
        String(64),
        ForeignKey("har_audit_sessions.id", ondelete="CASCADE"),
        index=True,
    )
    event_seq: Mapped[int] = mapped_column(Integer, default=0)
    occurred_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, index=True)
    track_id: Mapped[int] = mapped_column(Integer, index=True)
    global_person_id: Mapped[str | None] = mapped_column(String(64), index=True)
    reid_match_score: Mapped[float | None] = mapped_column(Float)
    frame_idx: Mapped[int | None] = mapped_column(Integer)
    bbox_json: Mapped[str | None] = mapped_column(Text)
    action_label: Mapped[str | None] = mapped_column(String(256))
    raw_label: Mapped[str | None] = mapped_column(String(256))
    confidence: Mapped[float | None] = mapped_column(Float)
    raw_confidence: Mapped[float | None] = mapped_column(Float)
    class_index: Mapped[int | None] = mapped_column(Integer)
    top_k_json: Mapped[str | None] = mapped_column(Text)
    all_probs_json: Mapped[str | None] = mapped_column(Text)
    ...
    session: Mapped["HarAuditSession"] = relationship("HarAuditSession", back_populates="events")


class HarPerson(Base):
    """Stable person identity (Re-ID registry)."""

    __tablename__ = "har_persons"
    ...
    n_appearances: Mapped[int] = mapped_column(Integer, default=0)
    first_seen_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
    last_seen_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, index=True)
    meta_json: Mapped[str | None] = mapped_column(Text)

    appearances: Mapped[list["HarPersonAppearance"]] = relationship(
        "HarPersonAppearance",
...
class HarPersonAppearance(Base):
    __tablename__ = "har_person_appearances"

    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    global_person_id: Mapped[str] = mapped_column(
        String(64),
        ForeignKey("har_persons.id", ondelete="CASCADE"),
        index=True,
    )
    ...
    person: Mapped["HarPerson"] = relationship("HarPerson", back_populates="appearances")


class HarTrackLabel(Base):
    """Human track-level verdict (har-research track_labels.csv)."""

    __tablename__ = "har_track_labels"

    id: Mapped[str] = mapped_column(String(64), primary_key=True)
    session_id: Mapped[str] = mapped_column(String(64), index=True)
    track_id: Mapped[int] = mapped_column(Integer, index=True)
    video: Mapped[str | None] = mapped_column(String(512))
    global_person_id: Mapped[str | None] = mapped_column(String(64))
    person_verdict: Mapped[str] = mapped_column(String(16))  # yes | no | unknown
    display_name: Mapped[str | None] = mapped_column(String(128))
    ...
    dominant_action: Mapped[str | None] = mapped_column(String(256))
    dominant_confidence: Mapped[float | None] = mapped_column(Float)


class HarHumanLabel(Base):
    """Crop-level HITL (har-research human_labels/labels.csv)."""

    __tablename__ = "har_human_labels"
    ...
    person_verdict: Mapped[str | None] = mapped_column(String(16))
    action_verdict: Mapped[str | None] = mapped_column(String(16))
    correct_label: Mapped[str | None] = mapped_column(String(256))
