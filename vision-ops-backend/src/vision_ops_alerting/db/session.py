...
def _migrate_sqlite_schema() -> None:
    ...
    with engine.connect() as conn:
        ...
        if "config_json" not in rule_cols:
            conn.execute(text("ALTER TABLE alert_rules ADD COLUMN config_json TEXT"))
...
def init_db() -> None:
    ...
    with SessionLocal() as db:
        ...
        db.flush()  # email_templates must exist before alert_rules FK inserts
