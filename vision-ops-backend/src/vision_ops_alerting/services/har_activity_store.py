...
def _is_video_url(url: str | None) -> bool:
    ...
    return any(lower.endswith(ext) for ext in _VIDEO_SUFFIXES)


def _preview_url_for_row(row: HarActivityLog) -> str | None:
    """JPEG snapshot paths only — never fall back to mock-video clip URLs."""
    url = (row.snapshot_url or "").strip()
    if url and not _is_video_url(url):
        return url
    return None


def _detections_to_actor(
    ...
    def _track_key(d: dict[str, Any]) -> str | None:
        tid = d.get("track_id", d.get("track_index"))
        return str(tid) if tid is not None else None

    def _display_name(d: dict[str, Any], track_id: str | None) -> str | None:
        name = d.get("display_name") or d.get("person_name")
        if name:
            return str(name)
        if d.get("global_person_id"):
            return str(d["global_person_id"])
        return f"Track #{track_id}" if track_id else None

    if prediction and prediction.get("track_id") is not None:
        track_str = str(prediction["track_id"])
        for d in detections:
            if _track_key(d) == track_str:
                return "operator", track_str, _display_name(d, track_str), count
        return "operator", track_str, f"Track #{track_str}", count

    with_action = [d for d in detections if d.get("action_label")]
    if with_action:
        best = max(with_action, key=lambda d: float(d.get("action_confidence") or 0))
        track_id = _track_key(best)
        return "operator", track_id, _display_name(best, track_id), count
...
def log_to_dict(row: HarActivityLog) -> dict[str, Any]:
    ...
    preview_url = _preview_url_for_row(row)
    return {
        "id": row.id,
...
def record_activity(
    ...
    *,
    ...
    actor_type, actor_track_id, actor_name, person_count = _detections_to_actor(detections, pred)
...
def analytics_model_performance(
    ...
    *,
    ...
    for mid, group in sorted(by_model.items()):
        ...
        sample = group[0]
        preview = next((_preview_url_for_row(r) for r in group if _preview_url_for_row(r)), None)
        model_rows.append(
            {
                "modelId": mid,
        ...
        combo_rows.append(
            {
                ...
                "previewUrl": _preview_url_for_row(sample)
                    or next((_preview_url_for_row(r) for r in group if _preview_url_for_row(r)), None),
