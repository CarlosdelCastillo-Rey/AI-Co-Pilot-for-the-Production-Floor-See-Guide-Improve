...
def _looks_like_image_url(url: str | None) -> bool:
    ...
    if (
        ...
        or "/artifacts/" in lower
        or "/api/vision/" in lower
        or "/api/alerting/" in lower
        or "/api/har/" in lower
    ):
        return True
    return False
...
def _thumbnail_from_context(ctx: IndustrialContext) -> str | None:
    ...
    if _looks_like_image_url(snap):
        return _resolve_thumbnail_url(snap)
    path = Path(snap).expanduser()
    if path.is_file() and "alert_snapshots" in path.parts:
        return f"{settings.public_base_url.rstrip('/')}/api/alerting/snapshots/{path.name}"
    return None
...
def _resolve_thumbnail_url(url: str | None) -> str:
    ...
    if trimmed.startswith(("http://", "https://")):
        ...
        if path.startswith("/api/alerting/") or path.startswith("/api/har/"):
            return f"{settings.public_base_url.rstrip('/')}{path}"
        if path.startswith("/api/vision/"):
            return f"{settings.public_api_base.rstrip('/')}{path}"
        return trimmed
    if trimmed.startswith("/api/vision/"):
        return f"{settings.public_api_base.rstrip('/')}{trimmed}"
    if trimmed.startswith("/api/alerting/") or trimmed.startswith("/api/har/"):
        return f"{settings.public_base_url.rstrip('/')}{trimmed}"
    return trimmed
...
def log_telegram_delivery(
    ...
    *,
    ...
    delivery = AlertDelivery(
        ...
        channel="telegram",
        template_id=template_id,
        to_emails_json=json.dumps(chat_ids),
        message_ids_json=json.dumps(message_ids) if message_ids else None,
        dry_run=dry_run,
        status=status,
        error_message=error_message,
...
def rule_to_dict(rule: AlertRule) -> dict:
    out = {
        ...
        "severity": (
            (rule.severity if rule.severity != "DISABLED" else "INFO")
            if rule.enabled
            else "DISABLED"
        ),
        "enabled": rule.enabled,
        "notifyEmail": rule.notify_email,
        "notifyTelegram": rule.notify_telegram,
    ...
    if rule.case_type == "har_action_detected":
        out["confidenceThreshold"] = rule_confidence_threshold(
            rule, settings.har_notify_confidence_threshold
        )
        har_label = rule_har_action_label(rule)
        if har_label:
            out["harActionLabel"] = har_label
        out["eventSeverity"] = rule_event_severity(rule)
        out["notifyInApp"] = rule_notify_in_app(rule)
    return out
