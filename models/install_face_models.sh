#!/usr/bin/env bash
# Download OpenCV YuNet + SFace ONNX models into models/ (not committed to git).
# Called automatically by ./run-local.sh on first run.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MODELS_DIR="$ROOT/models"
SFACE_DIR="$MODELS_DIR/face_recognition_sface"
YUNET_DIR="$MODELS_DIR/face_detection_yunet"
...
log "These files stay on your machine only (see models/README.md)."
