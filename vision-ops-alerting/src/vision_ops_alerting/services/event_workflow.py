...
def build_timeline_stats(db: Session, event_date: str | None = None) -> dict:
    ...
    open_count = sum(1 for e in events if (e.resolution_status or "OPEN") == "OPEN")
    ack_count = sum(1 for e in events if e.resolution_status == "ACKNOWLEDGED")
    resolved_count = sum(1 for e in events if e.resolution_status == "RESOLVED")
    false_pos_count = sum(1 for e in events if e.resolution_status == "FALSE_POSITIVE")
    open_critical = sum(
        1
        for e in events
